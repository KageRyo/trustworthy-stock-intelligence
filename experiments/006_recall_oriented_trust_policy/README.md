...
This experiment studies warning-policy tradeoffs using an existing threshold
sweep from the Temporal Transformer trust run. It does not retrain the model.

The goal is to identify candidate policies that improve alert recall and then
make the cost explicit through alert false alarm rate, coverage, and selective
risk.
...
- Recall-oriented candidates can raise alert recall materially versus the v1
  conservative policy.
