...
The CI `Watchlist-to-warning E2E` job runs a deterministic, network-free pipeline against an
...
Set `TSI_E2E_API_BINARY` to a built `services/api-gateway-go/cmd/server` binary when running without
`go run`. Set `TSI_E2E_ANALYSIS_PATH` to have the test write the analysis response for the frontend
Vitest contract check.
